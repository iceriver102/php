...
Class Datatable extends CI_Model
{
	...
	public function order()
	{
		...
		if ( isset( $this->getData['iSortCol_0'] ) )
		{
			...
			for ( $i=0 ; $i<intval( $this->getData['iSortingCols'] ) ; $i++ )
			{
				...
				{
					$sOrder .= "`".$this->aColumns[ intval( $this->getData['iSortCol_'.$i] ) ]."` ".
	...
	public function where($where='')
	{
		...
		if ( isset($this->getData['sSearch']) && $this->getData['sSearch'] != '' )
		{
			...
			for ( $i=0 ; $i<count($this->aColumns) ; $i++ )
			{
				...
				{
					$sWhere .= '`'.$this->aColumns[$i].'` LIKE "%'.mysql_real_escape_string( $this->getData['sSearch'] ).'%" OR ';
				}
			}
	...
	{
		$sQuery = 'SELECT FOUND_ROWS() as indexRow';
		$query=$this->db->query($sQuery);
		$aResultFilterTotal=$query->result_array();
		//print_r($aResultFilterTotal[0]['Total']);
		return $aResultFilterTotal[0]['indexRow'];		
	}
	public function total()
	{
		$sQuery = 'SELECT COUNT(`'.$this->sIndexColumn.'`) as Total
			FROM '.$this->sTable.$this->where();
		$g_query=$this->db->query($sQuery);
		$aResultTotal=$g_query->result_array();
		return $aResultTotal[0]['Total'];
	}
